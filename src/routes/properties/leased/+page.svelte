...
<svelte:head>
	<title>Leased Properties | Jax Test</title>
		...
		<div class="w-full lg:w-[450px] xl:w-[500px] h-full overflow-y-auto bg-white p-4 md:p-6 flex flex-col gap-4 custom-scrollbar shrink-0 relative z-10">
			...
			{#if properties.length > 0}
				{#each properties as property}
					<div class="group flex bg-white border border-zinc-200 hover:border-teal-500/50 rounded-sm overflow-hidden transition-all duration-300 shadow-sm hover:shadow-lg">
						
						<div class="w-2/5 md:w-1/3 h-auto relative overflow-hidden bg-zinc-100 shrink-0">
							<img 
								src={property.image} 
								alt={property.title}
						...
						<div class="w-3/5 md:w-2/3 p-4 flex flex-col justify-between">
							<div>
								<span class="text-teal-600 text-[9px] font-black uppercase tracking-widest block mb-1">
									{property.type || 'Retail'}
								</span>
								<h3 class="text-lg font-bold text-zinc-950 leading-tight mb-1 line-clamp-2">
									{property.title}
								</h3>
	...
	{#if isMobileMapOpen}
		...
		>
			<div class="bg-zinc-950 px-6 py-5 flex justify-between items-center shrink-0 border-b border-zinc-800 shadow-lg">
				<span class="text-white font-bold uppercase tracking-widest text-xs">Leased Properties Map</span>
				<button 
